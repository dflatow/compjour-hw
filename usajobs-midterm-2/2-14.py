from midterm_utils import get_all_CA_data, cleanmoney
...
def days_on_list(job, date_collected=DATE_COLLECTED):
    end_date = parse_start_date(job['EndDate'])
    return (end_date - date_collected).days
...
for job in sorted(jobs, key=days_on_list):
    ...
    min_salary = cleanmoney(job['SalaryMin'])

    if (days >= 0) and (days < 5) and (min_salary >= 90000):
